...
<form method="POST" action="?/register" use:enhance={({ result }) => {
	return async ({ update, apply, cancel }) => {
		// Check if result is a redirect (successful registration)
	...
	<h1 class="mt-2 px-2 py-3 text-center text-xl font-bold">Register</h1>
	<div class="flex flex-col px-5 py-2">
		...
		<label for="password-retype">Ulangi Password</label>
		<input type="password" id="password-retype" name="password-retype" required />
		<span class="mt-4"
			>Sudah punya akun?, masuk <a class="text-violet-400" href="/login">disini</a></span
		>
		<button
			type="submit"
